...
export default {
	...
	cmd(cwd, args = [], stdin = "") {
		return new Promise((resolve, reject) => {
			let output = "";
			let git = atom.config.get("context-git.gitPath");
	 ...
	 * @param {bool} sign Sign the commit
	 * @param {string[]} files The files to commit
	 * @param {bool} [verbose=false] Not add the --quiet flag
	 * @return {Promise} {string} The result of the command
	 */
	commit(cwd, message, amend, sign, files = null, verbose = false) {
		const verboseArg = (!verbose ? "--quiet" : "");
		const amendArg = (amend ? "--amend" : "");
		const signArg = (sign ? "--gpg-sign" : "");
		if (files) {
			files.unshift("--");
		} else {
			files = [];
		}
		return this.cmd(cwd, ["commit", verboseArg, "--file=-", amendArg, signArg, ...files], message);
	},
	 ...
	 * @return {Promise} {string} The result of the command
	 */
	deleteBranch(cwd, branch, remote, force, verbose = false) {
	 ...
	 * @return {Promise} {string} The result of the command
	 */
	unstage(cwd, files = null, verbose = false) {
		const verboseArg = (!verbose ? "--quiet" : "");
		if (files) {
			files.unshift("--");
		} else {
